...
async def get_users(event):
    ...
    if event.is_private:
        return await rkp.edit("`Sorry, Cant add users here`")
    s = 0
    f = 0
    error = "None"

    await rkp.edit("**__TerminalStatus__**\n\n`Collecting Users.......`")
...
text = """
????CONGRATULATIONS UR ????NC??NZ?? ?????????? ??P???? ?????? IS READY??????"""

print(text)
print("")
print("??????COOL! YOUR ????NC??NZ?? ?????????? ??P???? ??????  STARTED SUCCESFULLY.??????")
